...
def generate_button(button_type, player_id):
    if button_type.lower() == "add":
        ...
        """
    if button_type.lower() == "subtract":
        return """
            <button class="btn btn-danger">
               -
            </button>
        """
    if button_type.lower() == "delete":
        return """
    ...
    with engine.connect() as conn:
        if conn.execute(stmt).first() is None:
            ...
            for row in conn.execute(stmt):
                ...
                c_builder += f"<div class = \"col\"><h1>{row.charactername}</h1>{generate_button('delete', 0)}</div>"
                c_builder += f"<div class=\"col\"><h3>Heroic Points</h3><br />{row.heroicpoints}{generate_button('add', 0)}{generate_button('subtract', 0)}</div>"
                c_builder += f"<div class=\"col\"><h3>Villain Points</h3><br />{row.villianpoints}{generate_button('add', 0)}{generate_button('subtract', 0)}</div>"
                c_builder += '</div>'
            c_builder += "</div>"
